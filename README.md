...
- Standardprofilen är `gemini-3.1-flash-lite` med `MINIMAL` thinking och det versionsmärkta kontraktet `gemini-live-v6`. Varje collection-runda får en maskinläsbar återstående budget, och endast tools som fortfarande är tillåtna exponeras för modellen. Live måste både vara aktiverat på servern och bekräftas i varje request.
...
- Prompt v5 infördes efter att en v4-regressionskörning hittade rätt inventory-rotorsak men bara gav 3/5 direkt stödda claim-evidence-länkar. Verifieraren lättades inte. v6 behåller de skärpta evidenskraven och lägger till den verkliga återstående tool-budgeten i varje collection-runda.
...
- Två v6-försök med `gemini-3.5-flash-lite` nådde provider-timeout. Den stabila låg-latensprofilen `gemini-3.1-flash-lite` valdes därefter som utvecklingsstandard utifrån officiellt stöd för function calling/structured output och observerade smokes, inte utifrån modellnamnet.
- Med standardprofilen och riktig RAG-profil gav payment-scenariot rätt rotorsak/tjänst, giltiga citationer och ett faktiskt pgvector-hämtat runbookresultat på 6 057 ms. Det använde 3 modellanrop, 4 tool calls, 5 919 tokens och cirka 0,002656 USD i betalt listprisestimat.
...
- En modellleverantör med custom function tools och structured output. Nuvarande mätta standardprofil är `gemini-3.1-flash-lite` + `MINIMAL` thinking genom Googles officiella Java SDK. Modellen stöder [function calling och structured outputs](https://ai.google.dev/gemini-api/docs/models/gemini-3.1-flash-lite); prisestimat utgår från [Googles betalda standardlistpris](https://ai.google.dev/gemini-api/docs/pricing), medan faktisk free-tier-kostnad kan vara noll.
...
Den implementerade körgränsen är högst två collection-rundor, tre modellanrop, en global hard cap på åtta tool calls, högst tre tool calls per runda och 45 sekunders hard timeout. De striktare per-tool-gränserna ger i nuläget högst sex faktiska calls: ett metrics-anrop, två loggsökningar, två traces och en runbookhämtning. Första collection får högst 28 sekunder, en eventuell andra runda högst 8 sekunder och synthesis reserveras tid. Den sista synthesis-rundan saknar tools och följs endast av deterministisk verifiering. Modellen kan svara `insufficient_evidence`.
